...
import com.think.util.concurrent.SystemNanoClock;
 ...
 * <p>
 * Assumes single-writer principle and timers firing on processing thread. Low
 * (or NO) garbage.
 ...
 * <p>
 * Wheel is backed by arrays. Timer cancellation is O(1). Timer scheduling might
 * be slightly longer if a lot of timers are in the same tick. The underlying
 * tick contains an array. That array grows when needed, but does not currently
 * shrink.
 * <p>
 * Timer objects may be reused if desired, but all reuse must be done with timer
 * cancellation, expiration, and timeouts in consideration.
 * <p>
 * Caveats
 * <p>
 * Timers that expire in the same tick will not be ordered with one another. As
 * ticks are fairly large normally, this means that some timers may expire out
 * of order.
 */
public class TimerWheel {
	 ...
	 * 
	 * @param tickDuration
	 *            of each tick of the wheel
	 * @param timeUnit
	 *            for the tick duration
	 * @param ticksPerWheel
	 *            of the wheel. Must be a power of 2.
	 */
	 ...
	 * Construct a timer wheel for use in scheduling timers.
	 * <p>
	 * This constructor allows a custom function to return the current time instead
	 * of {@link System#nanoTime()}.
	 * 
	 * @param clock
	 *            to use for system time
	 * @param tickDuration
	 *            of each tick of the wheel
	 * @param timeUnit
	 *            for the tick duration
	 ...
	 *            of the wheel. Must be a power of 2.
	 */
	public TimerWheel(final NanoClock clock, final long tickDuration, final TimeUnit timeUnit,
		...
		if (tickDurationInNs >= (Long.MAX_VALUE / ticksPerWheel)) {
			throw new IllegalArgumentException(String.format("tickDuration: %d (expected: 0 < tickDurationInNs < %d",
	...
	public final class Timer {
		...
		public String toString() {
			return "Timer{" + "wheelIndex=\'" + wheelIndex + "\'" + ", tickIndex=\'" + tickIndex + "\'"
					+ ", deadline=\'" + deadline + "\'" + ", remainingRounds=\'" + remainingRounds + "\'" + "}";
		}
	}
}